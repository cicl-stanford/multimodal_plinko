...
def visualize(c, sim_data, save_images = False, make_video = False, video_name = 'test', unity=True):
    """
    Visualize the world. 
    ...
    if make_video:
        ...
        sp.call('ffmpeg -y -framerate 60 -i {ims}  -c:v libx264 -profile:v high -crf 10 -pix_fmt yuv420p {videoname}.mp4'.format(ims = img_dir+"/\'%05d.png\'", videoname = video_name), shell=True)
...
def snapshot(c, image_path="", image_name="world_snapshot", eye_pos=None, ball_pos=None, eye_data=None, unity_coordinates=False, ret_np=False):
    """
    ...
    Args:
        ...
        ball_pos: dict
            ...
            "y": float
                the y position
        ret_np: bool
            if true, will return an nd array visual observation of the world
    """
    # setup pygame 
    # pygame.init()
...
def transform_trial(aligned_shape_trial):
    ...
    if 'ball_final_position' in transformed_trial:
    ...
    ball_top_y = 350 + transformed_trial['ball_radius']
    _, new_ball_top_y =  convertCoordinate(350, ball_top_y)

    unity_radius = new_ball_top_y - 250
...
def show_unity_trial(trial_num, exp="inference", hole=None, ax=None, no_axis=True, folder_loc=None):
    ...
    if exp == "inference":
        ...
        if folder_loc is None:
            path = f"../../../figures/stimuli/inference/final/world_{trial_num}.png"
        else:
            path = f"{folder_loc}world_{trial_num}.png"

    elif exp == "prediction":

        filename = f"obstacles_world{trial_num}" if hole is None else f"hole{hole}_world{trial_num}"
        
        if folder_loc is None:
            path = f"../../../figures/stimuli/prediction/{filename}.png"
        else:
            path = f"{folder_loc}{filename}.png"

    elif exp == "mixed_test":

        filename = f"world_{trial_num}.png"

        if folder_loc is None:
            path = f"mixed_test/world_{trial_num}/{filename}"
        else:
            path = f"{folder_loc}{filename}"
            
    else:
        raise Exception("Experiment {} does not exist.".format(exp))
...
def graph_conditional_dist(ax, x, col, multiplier=8000, offset=490, unity_background=True, kde_method="FFT", label=None, precomputed=False, alpha=0.5, zorder=1):
    ...
    if not precomputed:
        ...
        if kde_method == "FFT":
            kde = FFTKDE(kernel="gaussian", bw=20).fit(x, weights=np.ones(len(x)))
    ...
    ax.fill(x_grid, p, color=col, alpha=alpha, label=label, zorder=zorder)
    ax.plot(x_grid, p, color=col, zorder=zorder)

    
    return ax


def draw_circle_drop(ax, hole_num, trial, colors = ["red", "blue", "green"]):
    col = colors[hole_num]

    pymunk_boxtop = trial['med'] + trial['height']/2

    hp_x_pymunk = trial['hole_positions'][hole_num]
    hp_x_unity = convertCoordinate(hp_x_pymunk, pymunk_boxtop)[0]

    circle_outline = Circle((hp_x_unity, 25), radius=17, color="black", zorder=2)
    ball = Circle((hp_x_unity, 25), radius=15, color=col, zorder=2)
...
def draw_circle_position(ax, position, col, radius=15, zord=2, alpha=1.0):

    circle_outline = Circle(position, radius=radius+2, color="black", zorder=zord, alpha=alpha)
    ball = Circle(position, radius=radius, color=col, zorder=zord, alpha=alpha)
...
def simulate_drop(trial, hole, drop_noise=None):
    
    if not (drop_noise is None):
        trial['drop_noise'] = drop_noise
    
    trial['hole_dropped_into'] = hole
    drop_sim = engine.run_simulation(trial)
    
    drop_pos = drop_sim['ball_position']
    drop_pos = [convertCoordinate(pos['x'], utils.flipy(trial, pos['y'])) for pos in drop_pos]
    drop_pos = [(x, y) for x,y in drop_pos]
    
    drop_fp = drop_pos[-1]
    
    unzipped_drop = list(zip(*drop_pos))
    xs = unzipped_drop[0]
    ys = unzipped_drop[1]
    
    return xs, ys, drop_fp


def draw_drop(ax, hole, trial, xs, ys, fp, alpha=1.0, colors = ["red", "blue", "green"]):
    
    color = colors[hole]
    
    draw_circle_drop(ax, hole, trial, colors=colors)
    ax.plot(xs, ys, "--", color=color, zorder=1, alpha=alpha)
    draw_circle_position(ax, fp, color, zord=2, alpha=alpha)
