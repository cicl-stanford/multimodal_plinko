...
var Config = function (condition, counterbalance) {
    ...
    this.parse_config = function (data) {
        a = data["stim"].slice(120, 122) //practice trials 
        b = data["stim"].slice(0, 120) //test trials 
        b = shuffle(b) //shuffle the test trials 
        this.trials = a.concat(b) //concatenate trials 
        this.videos = shuffle(data["videos"]);
