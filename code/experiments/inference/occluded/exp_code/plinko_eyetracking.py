...
def show_instruct(win):
    ...
    inst2 = ("Now that you have a sense for how the plinko box works, we can run some trials. In each trial, we will first completely cover up the box, and drop the ball into the box. You won't see the ball being dropped but hear the sounds that it makes as it collides with the obstacles, the walls, and as it lands in the sand at the bottom of the box.\n\nWe will then remove the top cover of the box, and it's your job to figure out in which hole the ball was dropped. You won't see the final location of the ball because we won't remove the bottom cover. You will answer on the keypad using the number keys 1, 2, and 3.")
    show_text(win, inst2)
